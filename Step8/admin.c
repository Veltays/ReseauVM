...
void SaiSieVehiculeHV(int Reference, struct VehiculeHV *UnRecord)
{
   ...
   strcpy(UnRecord->motorisation,"Essence");
...
int main()
{
   ...
   while (1)
   {
      ...
      case '4':
         ...
         scanf("%d", &UnRecord.Reference);
         if((RechercheHV("./VehiculesHV", UnRecord.Reference, &UnRecord)) > 0)
            {
               AfficheEnteteVehiculeHV();
               AfficheVehiculeHV(&UnRecord);
            }
      ...
      case '5':
         printf("NomClient :");
         fgets(Loueur.Acheteur, sizeof(Loueur.Acheteur), stdin);

         printf("Reference :");
         scanf("%d",&Loueur.Reference);


         printf("Quantité :");
         scanf("%d",&Loueur.Quantite);

         time_t current_time;
         current_time = time(NULL);  // Obtient l'heure actuelle en secondes depuis l'époque Unix
         Loueur.DateFacturation = current_time;

         if(ReservationHV("./VehiculesHV",Loueur.Reference,Loueur.Quantite))
            {
               FacturationHV("./FactureHV",Loueur.Acheteur,Loueur.DateFacturation,Loueur.Quantite,Loueur.Reference);
            }
      ...
      case '6':
         ...
         break;
      case '7':
         AProposServeurHV("V 8", "Yassine", "Gran");
         AProposServeurHV("V 8", "Ilyass", "Majdoub");
         break;
