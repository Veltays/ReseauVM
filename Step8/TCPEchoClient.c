...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        case 2:
            ...
            UneRequete.Date = current_time;
            UneRequete.Prix = rand() % 10000;

            //Envoie du message avec toute nos information
            if (write(sock, &UneRequete, sizeof(struct Requete)) != sizeof(struct Requete))
                DieWithError("send() sent a different number of bytes than expected");
            printf("Bytes send: %ld\n", sizeof(UneRequete));

            //Recupere la réponse du serveur
            if ((read(sock, &UneRequeteR, sizeof(struct Requete))) <= 0)
                DieWithError("recv() failed or connection closed prematurely");
            else
            {
